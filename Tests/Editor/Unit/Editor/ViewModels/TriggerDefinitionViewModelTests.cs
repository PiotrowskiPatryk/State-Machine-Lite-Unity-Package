...
{
    /// <summary>
    /// Unit tests for TriggerDefinitionViewModel.
    /// Tests construction, property binding, and disposal.
    /// </summary>
    [TestFixture]
    [Category("ViewModels")]
    public sealed class TriggerDefinitionViewModelTests
    {
        private TestPropertyContext<TriggerDefinition> _context;
        private TriggerDefinitionViewModel _viewModel;

        [SetUp]
        public void SetUp()
        {
            _context = SerializedPropertyTestHelper.CreateTestContext<TriggerDefinition>();
            _viewModel = new TriggerDefinitionViewModel(_context.Property);
        }
        ...
        {
            // Arrange & Act
            var standaloneVm = new TriggerDefinitionViewModel();
        ...
        [Test]
        public void Constructor_Copy_CopiesSerializedProperty()
        {
            // Arrange
            _viewModel.Name = "Original";
            _viewModel.Description = "Original Description";

            // Act
            var copyVm = new TriggerDefinitionViewModel(_viewModel);

            try
            {
                // Assert
                Assert.That(copyVm.Name, Is.EqualTo("Original"));
                Assert.That(copyVm.Description, Is.EqualTo("Original Description"));
            }
            finally
            {
                copyVm.Dispose();
        ...
        {
            // Arrange
            const string expectedValue = "trigger-id-123";
            var idProp = _context.Property.FindPropertyRelative(TriggerDefinition.ID_PROPERTY_NAME);
        ...
        {
            // Arrange
            const string expectedValue = "new-trigger-id";
        ...
        {
            // Arrange
            const string expectedValue = "Test Trigger";
            var nameProp = _context.Property.FindPropertyRelative(TriggerDefinition.NAME_PROPERTY_NAME);
        ...
        {
            // Arrange
            const string expectedValue = "New Trigger Name";
        ...
        {
            // Arrange
            const string expectedValue = "Trigger description";
            var descProp = _context.Property.FindPropertyRelative(TriggerDefinition.DESCRIPTION_PROPERTY_NAME);
        ...
        {
            // Arrange
            var expectedValue = typeof(MockTrigger).AssemblyQualifiedName;
            var typeProp = _context.Property.FindPropertyRelative(TriggerDefinition.TYPE_NAME_PROPERTY_NAME);
        ...
        public void Payload_ReturnsSerializedProperty()
        {
            ...
            Assert.That(payload.propertyPath, Does.Contain(TriggerDefinition.PAYLOAD_PROPERTY_NAME));
        }
        ...
        {
            // Arrange
            var sourceVm = new TriggerDefinitionViewModel();

            try
            {
                sourceVm.Id = "source-trigger-id";
                sourceVm.Name = "Source Trigger";
                ...
                // Assert
                Assert.That(_viewModel.Id, Is.EqualTo("source-trigger-id"));
                Assert.That(_viewModel.Name, Is.EqualTo("Source Trigger"));
