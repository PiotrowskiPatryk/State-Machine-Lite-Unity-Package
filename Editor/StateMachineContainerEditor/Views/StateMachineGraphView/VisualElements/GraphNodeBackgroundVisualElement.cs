...
namespace Dev.Cortez.StateMachines.Editor.StateMachineContainerEditor.Views.StateMachineGraphView.VisualElements
{
    /// <summary>
    ...
    public partial class GraphNodeBackgroundVisualElement : VisualElement
    {
        ...
        // ---- Pan bounds settings ----
        private bool _limitPanToBounds = false;
        ...
        public float BoundsX
        {
            ...
            set
            {
                if (Mathf.Approximately(_boundsX, value)) return;
                _boundsX = value;
                Pan = Pan;
            }
        }
        ...
        public float BoundsY
        {
            ...
            set
            {
                if (Mathf.Approximately(_boundsY, value)) return;
                _boundsY = value;
                Pan = Pan;
            }
        }
        ...
        public float BoundsWidth
        {
            ...
            set
            {
                ...
                if (Mathf.Approximately(_boundsWidth, v)) return;
                _boundsWidth = v;
                Pan = Pan;
            }
        }
        ...
        public float BoundsHeight
        {
            ...
            {
                var v = Mathf.Max(0f, value);
                if (Mathf.Approximately(_boundsHeight, v)) return;
                _boundsHeight = v;
                Pan = Pan;
            }
        }
        ...
        {
            get => new Rect(_boundsX, _boundsY, _boundsWidth, _boundsHeight);
            set
            {
        ...
        private VisualElement _panContainer;
        private VisualElement _zoomContainer;
        private VisualElement _overlayContainer;
        private Vector2Field _centerField;
        private Button _applyCenterButton;
        ...
        public GraphNodeBackgroundVisualElement()
        {
            ...
            _applyCenterButton = new Button(() =>
            {
                CenterOn(_centerField.value);
            })
            {
                text = "Apply Center"
            };
        ...
        private void RegisterCallbacks()
        {
            ...
            RegisterCallback<GeometryChangedEvent>(_ => { Pan = Pan; SyncCenterField(); });
        }
        ...
        {
            if (!LimitPanToBounds) return desiredPan;

            var viewportRect = contentRect;
            if (viewportRect.width <= 0f || viewportRect.height <= 0f) return desiredPan;

            var boundsRect = ContentBounds;
            var zoom = Zoom;

            float minPanX = viewportRect.xMax - zoom * (boundsRect.xMin + boundsRect.width);
            float maxPanX = viewportRect.xMin - zoom * boundsRect.xMin;
            float minPanY = viewportRect.yMax - zoom * (boundsRect.yMin + boundsRect.height);
            float maxPanY = viewportRect.yMin - zoom * boundsRect.yMin;

            float boundsScreenWidth = zoom * boundsRect.width;
            float boundsScreenHeight = zoom * boundsRect.height;

            float panX = boundsScreenWidth < viewportRect.width - 0.01f
                ? viewportRect.center.x - zoom * (boundsRect.xMin + boundsRect.width * 0.5f)
                : Mathf.Clamp(desiredPan.x, minPanX, maxPanX);

            float panY = boundsScreenHeight < viewportRect.height - 0.01f
        ...
        {
            var viewportRect = contentRect;
            if (viewportRect.width <= 0f || viewportRect.height <= 0f) return Vector2.zero;
            return (viewportRect.center - Pan) / Mathf.Max(Zoom, 0.0001f);
        }

        private void SyncCenterField()
        {
            if (_centerField == null) return;
